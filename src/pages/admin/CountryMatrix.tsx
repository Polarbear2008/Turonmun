...
const CountryMatrix = () => {
  ...
  const assignCountry = async (country: string, committeeId: string, delegateId: string) => {
    try {
      ...
      // Upsert to handle cases where a slot already has an assignment (avoids 409 duplicate key)
      const { error } = await supabase
        ...
        }, { onConflict: 'country,committee_id' });

      if (error) throw error;

      // Generate a temporary ID for local state (crypto.randomUUID() not available over HTTP)
